...
export class Matrix {
  ...
  set(col: number, row: number, v: Complex) {
    this.components[row][col] = v;
  }
  ...
  assert_bounds(other: Matrix) {
    if (this.width() != other.width() || this.height() != other.height()) {
      throw new Error(`Invalid size: ${this.width()}x${this.height()} vs ${other.width()}x${other.height()}`);
    }
  }

  assertSquare() {
    if (this.width() != this.height()) {
      throw new Error(`Invalid square dimensions ${this.width()} ${this.height()}`);
    }
  }
  ...
  trace(): Complex {
    if (this.width() != this.height()) {
      throw new Error(`invalid dimensions for trace ${this.width()}, ${this.height()}`);
    }
  ...
  normalize(): Matrix {
    return this.mulScalar(new Complex(1 / Math.sqrt(this.transpose().mulMatrix(this).at(0, 0).real), 0));
  }
}
